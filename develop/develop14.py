...
def parse_nwpw_block(nwinput_str):
    ...
    if dplot_match:
        ...
        if dplot_entries:
            output.append(f"dplot{{{' | '.join(dplot_entries)}}}")
...
def parse_calculation(nwinput_str):
    ...
    # Remove redundant "pspw" prefix
    clean_calculations = [c.replace("pspw ", "") for c in calculations]

    return f"calculation_type{{{'-'.join(clean_calculations)}}}" if clean_calculations else "calculation_type{}"
