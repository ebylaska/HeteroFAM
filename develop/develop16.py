...
def parse_nwpw_block(nwinput_str):
    """Parses the `nwpw` block for xc, mult, cutoff, pspspin, uterm, but NOT dplot."""
...
def parse_dplot(nwinput_str):
    """Extracts the `dplot` block and formats it for `calculation_type{pspw_dplot{...}}`."""
...
def parse_calculation(nwinput_str):
    """Parses task commands, set commands, and embeds dplot inside `calculation_type{}`."""
    ...
    # Step 4: Insert `pspw_dplot{}` block only once
    dplot_entry = parse_dplot(nwinput_str)
    if dplot_entry:
        calculations.insert(1, dplot_entry)

    # Step 5: Append `set` commands at the correct places
